"""
KV260 PYNQ driver – QAT 8-bit model (myproject_axi, ap_fixed<16,6> I/O)
========================================================================
Model   : BRAM-Opt v2 CNN, QAT 8-bit weights  (ap_fixed<8,5> kernels)
Accuracy: 80.89 %
...
Usage
-----
    from drivers.driver_qat_8bit import SpeakerRecogQAT8Overlay
    overlay = SpeakerRecogQAT8Overlay('bitfiles/qat_8bit/design_1_wrapper.bit')
...
class SpeakerRecogQAT8Overlay(Overlay):
    """
    PYNQ overlay driver for the QAT 8-bit speaker-recognition accelerator.

    Parameters
    ----------
    bitfile_name : str
        Path to the .bit file (e.g. 'bitfiles/qat_8bit/design_1_wrapper.bit').
    download : bool
    ...
    def predict(self, x: np.ndarray, profile: bool = False) -> np.ndarray:
        ...
        if profile:
            ...
            print(f"QAT-8 inference: {dt*1e3:.2f} ms  ({1/dt:.1f} inf/s)")
