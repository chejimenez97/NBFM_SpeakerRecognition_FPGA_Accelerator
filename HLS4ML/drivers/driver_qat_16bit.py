"""
KV260 PYNQ driver – QAT 16-bit model (myproject_axi, ap_fixed<16,6> I/O)
=========================================================================
Model   : BRAM-Opt v2 CNN, QAT 16-bit weights  (ap_fixed<16,7> kernels)
Accuracy: 91.26 %
...
class SpeakerRecogQAT16Overlay(Overlay):
    """
    PYNQ overlay driver for the QAT 16-bit speaker-recognition accelerator.

    Parameters
    ----------
    bitfile_name : str
        Path to the .bit file (e.g. 'bitfiles/qat_16bit/design_1_wrapper.bit').
    download : bool
    ...
    def predict(self, x: np.ndarray, profile: bool = False) -> np.ndarray:
        ...
        if profile:
            ...
            print(f"QAT-16 inference: {dt*1e3:.2f} ms  ({1/dt:.1f} inf/s)")
